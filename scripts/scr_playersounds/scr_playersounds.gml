function scr_playersounds() //scr_playersounds
{
    with (obj_player)
    {
        if (state == (79 << 0) && (!audio_is_playing(sfx_mach1)) && grounded)
        {
            mach1snd = audio_play_sound(sfx_mach1, 1, false)
            sfx_gain(mach1snd)
        }
        else if (state != (79 << 0) || (!grounded) || move == (-xscale))
            audio_stop_sound(mach1snd)
        if ((sprite_index == spr_mach || state == (26 << 0)) && (!audio_is_playing(sfx_mach2)))
        {
            mach2snd = audio_play_sound(sfx_mach2, 1, false)
            sfx_gain(mach2snd)
        }
        else if (sprite_index != spr_mach && state != (26 << 0))
            audio_stop_sound(mach2snd)
        if ((state == (94 << 0) || sprite_index == spr_mach3boost) && sprite_index != spr_crazyrun && (!audio_is_playing(sfx_mach3)))
        {
            mach3snd = audio_play_sound(sfx_mach3, 1, false)
            sfx_gain(mach3snd)
        }
        else if ((state != (94 << 0) && sprite_index != spr_mach3boost) || sprite_index == spr_crazyrun)
            audio_stop_sound(mach3snd)
        if (state == (27 << 0) && (!audio_is_playing(sfx_knightslide)))
        {
            knightslide = audio_play_sound(sfx_knightslide, 1, false)
            sfx_gain(knightslide)
        }
        else if (state != (27 << 0) && audio_is_playing(sfx_knightslide))
        ...
        if (state != (75 << 0) && audio_is_playing(superjumpprepsnd))
            audio_stop_sound(superjumpprepsnd)
        if (state != (75 << 0) && audio_is_playing(superjumpholdsnd))
        ...
        if (audio_is_playing(suplexdashsnd) && state != (31 << 0))
