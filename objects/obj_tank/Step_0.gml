...
if (state == (111 << 0) && lasthp != hp && (!tired) && grounded)
{
    tired = 1
    stunned = 10
    lasthp = hp
...
if (stuntouchbuffer > 0)
    stuntouchbuffer--
if ((!instance_exists(spawnenemyID)) && state == (107 << 0))
{
    state = (99 << 0)
...
{
    if (state != (98 << 0) && bombreset <= 0 && global.stylethreshold >= 3 && state == (107 << 0))
    {
        state = (98 << 0)
    ...
    else if (x != player.x && state != (98 << 0) && grounded && bombreset <= 0)
    {
        if (state == (107 << 0))
        {
            state = (102 << 0)
