...
if (global.pizzadelivery && ds_list_find_index(global.saveroom, id) == -1)
{
    ...
    if (!instance_exists(obj_cutscene_handler))
    {
        ...
        with (instance_create(0, 0, obj_cutscene_handler))
        {
            ...
            depth = (other.depth - 1)
            scene_info = [[cutscene_customer_check, door.gnome_id, player, door], [cutscene_customer_start, player], [cutscene_move_player, player, (door.x + (door.player_standx * (-door_xscale))), 6, 16], [cutscene_set_sprite, player, player.spr_idle, 0.35, door_xscale], [cutscene_customer_create, xx, yy, other.spr_idle, other.spr_happy, (-door_xscale)], [cutscene_wait, 45], [cutscene_customer_end, player, 401]]
        }
    }
}
