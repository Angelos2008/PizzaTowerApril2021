event_inherited()
state = (118 << 0)
ds_map_set(player_hurtstates, (31 << 0), 30)
ds_map_set(player_hurtstates, (30 << 0), 50)
ds_map_set(player_hurtstates, (80 << 0), 20)
ds_map_set(player_hurtstates, (94 << 0), 30)
ds_map_set(boss_hurtstates, (125 << 0), 30)
ds_map_set(boss_hurtstates, (60 << 0), 30)
ds_map_set(boss_hurtstates, (129 << 0), 60)
ds_map_set(boss_hurtstates, (53 << 0), 200)
ds_map_set(boss_hurtstates, (84 << 0), 30)
stunfallspr = spr_pepperman_hurtplayer
walkspr = spr_pepperman_move
idlespr = spr_pepperman_idle
spr_dead = spr_pepperman_timesup
tauntspr = spr_pepperman_taunt
fallspr = spr_pepperman_fall
defeatplayerspr = spr_pepperman_shoulderstart
normal_spd = 6
shoulder_spd = 12
attack_max[0] = 120
attack_max[1] = 80
attack_max[2] = 0
attack_max[3] = 0
attack_max[4] = 0
attack_max[5] = 0
attack_cooldown = attack_max[0]
targetstunned = 0
targetstunnedminus[0] = 80
targetstunnedminus[1] = 150
targetstunnedminus[2] = 200
targetstunnedminus[3] = 200
targetstunnedminus[4] = 200
targetstunnedminus[5] = 200
targetxscale = 1
phase = 1
max_phase = 6
max_hp = (500 * max_phase)
hp = max_hp
xscale = 1
jumping_pepper = 0
shoulderbash_fakeout = 0
groundpound_fakeout = 0
groundpound_readjust = 3
groundpound_readjust_max = 16
groundpound_readjust_buffer = groundpound_readjust_max
readjusting = 0
superjumping_pepper = 0
shoulderturns = 3
target_x = 0
shoulderpound_fakeout = 0
shoulderpound_buffer = 0
shoulderpound_max = 60
chooseparry_max[0] = 600
chooseparry_max[1] = 480
chooseparry_max[2] = 300
chooseparry_max[3] = 300
chooseparry_max[4] = 300
chooseparry_max[5] = 300
chooseparry_random = 4
chooseparry_buffer = chooseparry_max[(phase - 1)]
taunt_buffer = 0
taunt_max = 25
fistmatch_buffer = 0
fistmatch_max = 360
fist_buffer = 0
fist_max = 20
fist_dmg = 10
fist_dmg_player = 20
superattack_hpthreshold = 500
superattack_buffer = 0
superattack_max = 360
function player_hurt(argument0) //player_destroy_gml_Object_obj_pepperman_Create_0
{
    self.SUPER_player_destroy(argument0)
}

function player_hurt(argument0) //boss_destroy_gml_Object_obj_pepperman_Create_0
{
    hitstate = (0 << 0)
    self.SUPER_boss_destroy(argument0)
    with (obj_peppermanbrick)
        instance_destroy()
    targetstunned = 1000
    stunned = 1000
}

function player_hurt(argument0, argument1) //boss_hurt_gml_Object_obj_pepperman_Create_0
{
    if (targetstunned > 0)
    {
        targetstunned -= targetstunnedminus[(phase - 1)]
        attack_cooldown = 0
        if (targetstunned < 0)
            targetstunned = 1
    }
    else
        targetstunned = 150
    self.SUPER_boss_hurt(argument0, argument1)
    targetxscale = (-argument1.xscale)
}

function player_hurt(argument0) //boss_hurt_noplayer_gml_Object_obj_pepperman_Create_0
{
    if (targetstunned > 0)
    {
        targetstunned -= targetstunnedminus[(phase - 1)]
        attack_cooldown = 0
        if (targetstunned < 0)
            targetstunned = 1
    }
    else
        targetstunned = 150
    self.SUPER_boss_hurt_noplayer(argument0)
}

function player_hurt(argument0, argument1) //player_hurt_gml_Object_obj_pepperman_Create_0
{
    if (argument1.state != (61 << 0) || argument1.parry_inst == -4)
    {
        var _prevstate = state
        self.SUPER_player_hurt(argument0, argument1)
        if (_prevstate == (125 << 0) || _prevstate == (129 << 0) || _prevstate == (60 << 0) || _prevstate == (53 << 0))
        {
            with (obj_camera)
            {
                shake_mag = 3
                shake_mag_acc = (3 / room_speed)
            }
            hitstate = (111 << 0)
            stunned = 100
            hitvsp = -4
            hithsp = ((-image_xscale) * 8)
        }
        else if (_prevstate == (61 << 0))
        {
            with (obj_camera)
            {
                shake_mag = 3
                shake_mag_acc = (3 / room_speed)
            }
            sprite_index = spr_pepperman_throw
            image_index = 0
            hitstate = (120 << 0)
            hitvsp = 0
            hithsp = 0
            movespeed = 8
        }
        else
        {
            hithsp = 0
            hitvsp = -4
            hitstate = (0 << 0)
        }
    }
    else if (state == (53 << 0))
    {
        with (argument1)
        {
            if (state == (110 << 0) || state == (43 << 0))
            {
                x = hitX
                y = hitY
            }
            if (other.state == (110 << 0) || other.state == (43 << 0))
            {
                other.x = hitX
                other.y = hitY
            }
            sprite_index = spr_idle
            state = (134 << 0)
            hitX = x
            hitY = y
            hsp = 0
            vsp = 0
            movespeed = 0
            other.sprite_index = other.idlespr
            other.state = (134 << 0)
            other.image_xscale = (-xscale)
            other.hitX = (x + (xscale * 16))
            other.hitY = y
            other.hsp = 0
            other.vsp = 0
            other.fistmatch_buffer = other.fistmatch_max
            camera_zoom(0.6, 0.1)
        }
    }
}

